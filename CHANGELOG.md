...
- **Legend and truncation strip.** A clickable legend above the plot toggles per-series (or per-slice) visibility — hidden entries render dimmed and struck — alongside a "by <column>" tag and a "Showing …" note whenever the category, series, or source-row caps trimmed the data. In the sidebar's Values list, auto-selected numeric columns render checked so auto mode never reads as "nothing selected"; toggling materializes the explicit selection.
